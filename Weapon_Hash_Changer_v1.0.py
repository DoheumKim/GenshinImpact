...
def load_excel(select_list):
    ...
    if   select_list[0] == '한손검':        #무기종류별 무기개수, 무기 셀위치 설정
        if   select_list[1] == 5:WP_num,WP_start = 10,2
        elif select_list[1] == 4:WP_num,WP_start = 23,13
        elif select_list[1] == 3:WP_num,WP_start = 6,37
        ...
        with open(ini_file, "r", encoding="utf-8") as f:
            ...
            for i,l in enumerate(lines):
                if '[TextureOverride' in lines[i] and not '$active = 1' in lines[i+2]:
                    ...
                    result = result.replace(f"{mod_data[num]['hash']}",f";이전해쉬: {mod_data[num]['hash'][7:]}hash = {new_hash}\n")
                    mod_data[num]['hash'] = ''
