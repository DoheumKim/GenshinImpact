...
def main():
    ...
    while True:
        ...
        if load_ws[f'E{str(i)}'].value == None:break    #원소 없으면 break
            ...
            char[load_ws[f'A{str(i)}'].value] = [load_ws[f'C{str(i)}'].value,load_ws[f'E{str(i)}'].value]   #char == {'캐릭터':[ib,원소],'캐릭터2':[ib2,원소2]}
            a += 1
            i += 1
...
$swapvar = 1,2,3,4,5,6,7'
    ...
    for i in range(len(list(char.keys()))):
        result += '\n'
        result += f'[TextureOverride{list(char.keys())[i]}Position]\n'
        result += f'hash = {list(char.values())[i][0]}\n'
        result += 'match_priority = 1'
        result += '\n'
        if 'Traveler' in list(char.keys())[i]:result += '$swapvar = $MainChar\n'
        else:result += f'$swapvar = {list(char.values())[i][1]}\n'
        result += '\n'
